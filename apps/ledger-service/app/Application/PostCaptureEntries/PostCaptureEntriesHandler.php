...
final class PostCaptureEntriesHandler
{
    public function handle(PostCaptureEntriesCommand $command): LedgerTransaction
    {
        $idempotencyKey = "capture:{$command->paymentId}";
        ...
        $feesAccount = $command->feeAmount > 0
        ...
        try {
            return DB::transaction(function () use ($command, $idempotencyKey, $escrowAccount, $merchantAccount, $feesAccount): LedgerTransaction {
                ...
                LedgerEntry::create([
                    ...
                    'amount' => $command->amount - $command->feeAmount,
                    'currency' => $command->currency,
                ]);
